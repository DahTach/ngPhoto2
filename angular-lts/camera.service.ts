...
export class CameraService {
  ...
  readonly state = this.camera.state as BehaviorSubject<CameraState>;
  readonly isConnected = this.state.pipe(map((state: CameraState) => state === CameraState.READY)); // Fix: Explicitly type 'state' as 'CameraState'
  private previousSignificantState = CameraState.DISCONNECTED;

  constructor(private alertService: AlertService) {

    this.state.subscribe(state => {
      if (this.shouldShowAlert(state)) {
        this.showStateAlert(state);
        this.previousSignificantState = state;
      }
    })
